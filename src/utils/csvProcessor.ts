...
export const parseCSVFile = (file: File): Promise<ParsedFile> => {
    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        try {
          ...
          const duplicateRefs = findDuplicateReferences(validData);
...
const findDuplicateReferences = (data: Transaction[]): string[] => {
